...
learning_rate = 0.001
latent_dim = 10
beta = 1.0 # Pixel intensity factor
...
def compute_loss(model, x):
    ...
    x_pos = torch.zeros(batch_size,2,num_pixels).cuda()
    x_pos = x[:,0,:2,:] # Removes the channel dimension to make the following calculations easier
    x_int = torch.zeros(batch_size,num_pixels).cuda()
    x_int = x[:,0,2,:] # Removes the channel dimension to make the following calculations easier

    x_pos = x_pos.view(batch_size, 2, 1, num_pixels) # Changes the dimension of the tensor so that dist is the distance between every pair of input and output pixels

    x_decoded_pos = torch.zeros(batch_size,2,num_pixels).cuda()
    x_decoded_pos = x_decoded[:,0,:2,:] # Removes the channel dimension to make the following calculations easier
    x_decoded_int = torch.zeros(batch_size,num_pixels).cuda()
    x_decoded_int = x_decoded[:,0,2,:] # Removes the channel dimension to make the following calculations easier

    x_decoded_pos = x_decoded_pos.view(batch_size, 2, num_pixels, 1) # Changes the dimension of the tensor so that dist is the distance between every pair of input and output pixels
    ...
    aux_idx = num_pixels * torch.arange(batch_size).cuda() # This will help getting the right pixel intensities
    aux_idx = aux_idx.view(batch_size, 1)
    aux_idx = torch.repeat_interleave(aux_idx, num_pixels, axis=-1)
    ...
    eucl = ieo.values + oei.values + beta * (torch.pow((x_decoded_int - get_x_int), 2) + torch.pow((x_int - get_x_decoded_int), 2)) # Symmetrical euclidean distances
...
plt.legend()
#plt.savefig('loss_superpixel_euclsquared_alltensorloss_plotloss_1int_27sig_beta'+str(int(beta))+'_latentdim'+str(latent_dim)+'_'+str(num_epochs)+'epochs.png')
#plt.savefig('paper_super_loss_jrway.png')

# Save the model
#torch.save(model.state_dict(), 'model_superpixel_euclsquared_alltensorloss_1int_27sig_beta'+str(int(beta))+'_latentdim'+str(latent_dim)+'_'+str(num_epochs)+'epochs.pt')
#torch.save(model.state_dict(), 'paper_super_model_jrway')
...
#fig.savefig('image_MNISTsuperpixel_train_euclsquared_alltensorloss_plotloss_27int_27sig_beta'+str(int(beta))+'_latentdim'+str(latent_dim)+'_'+str(num_epochs)+'epochs.png')
...
cbar_ax = fig.add_axes([0.85, 0.115, 0.02, 0.76])
fig.colorbar(img, cax=cbar_ax)
#fig.savefig('image_MNISTsuperpixel_test_euclsquared_alltensorloss_plotloss_27int_27sig_beta'+str(int(beta))+'_latentdim'+str(latent_dim)+'_'+str(num_epochs)+'epochs.png')
...
cbar_ax = fig.add_axes([0.85, 0.115, 0.02, 0.76])
fig.colorbar(img, cax=cbar_ax)
#fig.savefig('image_superpixel_train_euclsquared_alltensorloss_plotloss_27int_27sig_beta'+str(int(beta))+'_latentdim'+str(latent_dim)+'_'+str(num_epochs)+'epochs.png')
...
#fig.savefig('image_superpixel_test_euclsquared_alltensorloss_plotloss_27int_27sig_beta'+str(int(beta))+'_latentdim'+str(latent_dim)+'_'+str(num_epochs)+'epochs.png')
...
#fig.savefig('image_superpixel_gaussian_euclsquared_alltensorloss_plotloss_27int_27sig_beta'+str(int(beta))+'_latentdim'+str(latent_dim)+'_'+str(num_epochs)+'epochs.png')
