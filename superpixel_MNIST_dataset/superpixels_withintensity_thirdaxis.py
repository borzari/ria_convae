...
def compute_loss(model, x):
    ...
    x_pos = torch.zeros(batch_size,3,25).cuda() # Input pixels features (x, y, I)
    x_pos = x[:,0,:,:]

    x_pos = x_pos.view(100, 3, 1, 25) # This step is important for the calculation of dist

    # This removes the channel dimension from the tensor and makes pdist easier to use
    x_decoded_pos = torch.zeros(batch_size,3,25).cuda() # Output pixels features (x, y, I)
    x_decoded_pos = x_decoded[:,0,:,:]

    x_decoded_pos = x_decoded_pos.view(batch_size, 3, 25, 1) # This and the next line are also important steps for the calculation of dist
    ...
    eucl = ieo.values + oei.values # This is summing the distances between the pixels using their intensities as the third axis in a 3D space.
...
plt.legend()
plt.savefig('loss_superpixel_thirdaxis_27int_latentdim'+str(latent_dim)+'_'+str(num_epochs)+'epochs.png')

# Save the model
torch.save(model.state_dict(), 'model_superpixel_thirdaxis_27int_latentdim'+str(latent_dim)+'_'+str(num_epochs)+'epochs.pt')
...
fig.savefig('image_MNISTsuperpixel_train_thirdaxis_27int_latentdim'+str(latent_dim)+'_'+str(num_epochs)+'epochs.png')
...
cbar_ax = fig.add_axes([0.85, 0.115, 0.02, 0.76])
fig.colorbar(img, cax=cbar_ax)
fig.savefig('image_MNISTsuperpixel_test_thirdaxis_27int_latentdim'+str(latent_dim)+'_'+str(num_epochs)+'epochs.png')
...
cbar_ax = fig.add_axes([0.85, 0.115, 0.02, 0.76])
fig.colorbar(img, cax=cbar_ax)
fig.savefig('image_superpixel_train_thirdaxis_27int_latentdim'+str(latent_dim)+'_'+str(num_epochs)+'epochs.png')
...
fig.savefig('image_superpixel_test_thirdaxis_27int_latentdim'+str(latent_dim)+'_'+str(num_epochs)+'epochs.png')
...
fig.savefig('image_superpixel_gaussian_thirdaxis_27int_latentdim'+str(latent_dim)+'_'+str(num_epochs)+'epochs.png')
